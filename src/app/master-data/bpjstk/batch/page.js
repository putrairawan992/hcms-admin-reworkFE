...
import useBpjstkBatch from "./useBpjstkBatch";
import { SelectField } from "@/app/components/atoms";
import { monthLabelOptions, yearOptions } from "@/shared/general";
import { isEmpty } from "lodash";

const MasterDataBPJSTKBatch = () => {
  const router = useRouter();
  const { data, loading, filters, onChangeSelect, onPressDetail } = useBpjstkBatch();
  ...
  return (
    <Box style={styles.container}>
      <Flex>
        <Box style={styles.header2} paddingX={12} paddingY={6} onClick={() => router.push('/master-data/bpjstk')}>
          <Text fontWeight='900' fontSize={22} color='#AE445A' textAlign='center' alignSelf='center'>Master Data - BPJSTK</Text>
        </Box>
...
export default MasterDataBPJSTKBatch;
