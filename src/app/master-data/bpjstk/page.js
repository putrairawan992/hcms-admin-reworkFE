...
import useBpjstk from "./useBpjstk";
import columns from "./columns";
import { DataTables } from "@/app/components/molecules";

const MasterDataBPJSTK = () => {
  const router = useRouter();
  const { data, loading, page, totalData, keyword, modalOpen, onChangeText, onHandleSync, onChangePagination, toggleModal, goToSpreadSheet } = useBpjstk();

  return (
    <Box style={stylesheet.container}>
      <Flex>
        <Box style={stylesheet.header} paddingX={12} paddingY={6}>
          <Text style={stylesheet.headerTitle}>Master Data - BPJSTK</Text>
        </Box>
        <Box style={stylesheet.header2} paddingX={12} paddingY={6} onClick={() => router.push('/master-data/bpjstk/batch')}>
...
export default MasterDataBPJSTK;
