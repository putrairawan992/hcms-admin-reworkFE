...
import usePajakBatchDetail from "./usePajakBatchDetail";
...
const MasterDataBPJSKESBatchDetail = () => {
  const { data, loading, page, totalData, keyword, modalOpen, month, years, onChangeText, onHandleSync, onChangePagination, toggleModal } = usePajakBatchDetail();

  return (
    <Box style={stylesheet.container}>
      <Flex>
        <Box paddingX={8} paddingY={8}>
          <Text fontSize={22} fontWeight={900} color='#AE445A'>Master Data PAJAK - Batch {month || '-'} {years || '-'}</Text>
        </Box>
      </Flex>
      <Box paddingX={8}>
