...
const MasterDataPajak = () => {
  ...
  const { data, loading, page, totalData, keyword, modalOpen, onChangeText, onHandleSync, onChangePagination, toggleModal, goToSpreadSheet } = usePajak();

  return (
    <Box style={stylesheet.container}>
      <Flex>
        <Box style={stylesheet.header} paddingX={12} paddingY={6}>
          <Text style={stylesheet.headerTitle}>Master Data - PAJAK</Text>
        </Box>
        <Box style={stylesheet.header2} paddingX={12} paddingY={6} onClick={() => router.push('/master-data/pajak/batch')}>
