...
const MasterDataMergeProduct = () => {
  ...
  const { data, loading, page, totalData, keyword, onChangeText } = useMergeProduct();

  return (
    <Box style={stylesheet.container}>
      <Flex>
        <Box style={stylesheet.header}>
          <Text style={stylesheet.headerTitle}>Master Data - Merge Product</Text>
        </Box>
        <Box style={stylesheet.header2} onClick={() => router.push('/master-data/merge-product/batch')}>
