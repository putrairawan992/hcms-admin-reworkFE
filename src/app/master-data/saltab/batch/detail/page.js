...
const MasterDataSALTABBatchDetail = () => {
  const { data, loading, page, totalData, keyword, modalOpen, month, years, onChangeText, onHandleSync, onChangePagination, toggleModal } = useSaltabBatchDetail();

  return (
    <Box style={stylesheet.container}>
      <Flex>
        <Box paddingX={8} paddingY={8}>
          <Text fontSize={22} fontWeight={900} color='#AE445A'>Master Data SALTAB - Batch {month || '-'} {years || '-'}</Text>
        </Box>
      </Flex>
      <Box paddingX={8}>
