...
const FormFieldsOfferingLatterKhusus = ({ loading = false, onClick = noop }) => {
  ...
  return (
    <Box>
      <Gap height={4} />
      <Text fontSize={16} fontWeight='bold' color='#AE445A'>Offering Letter Skema Khusus</Text>
      <Gap height={4} />
      <Flex>
        <Box flex={1}>
          {dataFormOfferingLatterKhusus.map((item) => {
            return (
      ...
      <Flex flex={1} justifyContent='flex-end'>
        <Button paddingX={10} borderRadius={10} background='linear-gradient(90deg, #f39f5a 0%, #ae445a 100%)' fontSize={12} color='#FFFFFF' fontWeight='bold' onClick={() => onHandleSubmit()}>
          {loading ? <Image src="/images/loading-white.gif" width={6} height={6} /> : 'Save'}
        </Button>
      </Flex>
    </Box>
  );
