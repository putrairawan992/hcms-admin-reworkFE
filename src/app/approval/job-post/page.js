...
import { monthLabelOptions, statusJobPostOptions, statusRemunOptions, yearOptions } from '@/shared/general';
...
const ApprovalJobPost = () => {
  const { data, filters, loading, productDigitalData, text, isOpen, isApprove, selectedId, toggleModal, onClose, onChangeSelect, onPressDetails } = useApprovalJobPost();
  ...
  return (
    <Box className={styles['job-post-container']}>
      ...
      <Flex className={styles['job-post-filter-container']}>
        <SelectField options={yearOptions} label='Tahun' slug='years' onChange={onChangeSelect} value={filters.years} />
        <SelectField options={monthLabelOptions} label='Bulan' slug='month' onChange={onChangeSelect} value={filters.month} />
        <SelectField options={productDigitalData} label='Digital Product' slug='product_digital_name' onChange={onChangeSelect} value={filters.product_digital_name} />
        <SelectField options={statusJobPostOptions} label='Status' slug='status' onChange={onChangeSelect} value={filters.status} />
      </Flex>
